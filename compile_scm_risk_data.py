...
# Sheets config loading from scm_master_config.json
ENABLE_SHEETS_FETCH = False
...
if os.path.exists(CONFIG_PATH):
    ...
    try:
        ...
        # Parse sheet-fetching rules if present
        sheets_config = config_data.get("sheets_config", {})
        ENABLE_SHEETS_FETCH = sheets_config.get("enable_sheets_fetch", False)
        CURRENCY_SS_ID = sheets_config.get("currency_ss_id", CURRENCY_SS_ID)
        
        print(f"  - Successfully loaded {len(VENDOR_MAP)} vendor rules and {len(KEYWORD_MAP)} theme rules.")
        print(f"  - Sheets Fetch: {ENABLE_SHEETS_FETCH} (ID: {CURRENCY_SS_ID})")
...
def compile_management_stability():
    ...
    # 1. 경영안정 조기경보이력 (좌측 1fr, 7개 열, 5개 행)
    warning_history = [
        ["센싱일자", "협력업체", "품목군", "위험 분류", "진행 상태", "세부 내용", "Risk"],
        ["2026-09-04", "Toshiba Memory", "Memory", "M&A/지배구조", "진행중", "Kioxia 합병 추진 실사 및 미국 사모펀드 인수 실사 진행에 따른 공급선 주도권 변동 주시", "중"],
        ["2026-09-01", "STMicroelectronics", "IC", "노사분규", "주시", "필리핀 현지 생산 공장 임금 인상 조율 결렬 및 부분 노조 태업으로 인한 OSAT 물량 인도 차질 우려", "상"],
        ["2026-08-27", "TDK Corporation", "Passive", "승계/경영권", "완료", "창업주 일가 은퇴 및 전문경영인 이사회 승계 절차 돌입, 중장기 부품 사업 포트폴리오 영향 모니터링", "하"],
        ["2026-08-22", "Yageo Corporation", "Passive", "경영난", "진행중", "단기 자금 유동비율 급감 및 대만 현지 은행단 긴급 워크아웃 채무 유예 조정안 실사 착수", "상"],
        ["2026-08-15", "Murata Mfg", "Passive", "생산차질", "주시", "일본 내륙 정밀 제련 자회사 가동 제한 권고 및 핵심 원자재 공급 라인 정밀 소독 다운타임", "중"]
    ]
...
def compile_cost_market_risk():
    ...
    live_currency = None
    if ENABLE_SHEETS_FETCH:
        live_currency = fetch_public_sheet_csv(CURRENCY_SS_ID, "환율원장")
    else:
        print("  - Sheets fetch is disabled in scm_master_config.json. Bypassing remote fetching and using default market-aligned fallback.")
